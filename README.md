...
Counterexample classification for Signal Temporal Logic (STL) specifications.
...
python -m ceclass.examples.autotrans --data /path/to/AT1_traces.mat --strategy long_bs --k 2 --device cuda

# Run paper experiments (all benchmarks, k=1..4, both strategies)
python run_paper_experiments.py --device cuda --output-dir results_100

# Generate visualizations (lattice diagrams + covered formula listings)
python visualize_results.py --device cuda
